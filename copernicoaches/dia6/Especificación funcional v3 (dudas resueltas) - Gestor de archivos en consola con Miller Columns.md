...
- Durante el scroll vertical, la **cabecera de ruta permanece fija** y solo se desplaza el cuerpo de columnas.
- Cuando el listado excede el viewport, la visualización se comporta en **bloques dinámicos visibles**:
  - Al navegar hacia abajo, los elementos superiores salen de ventana y aparecen nuevos elementos inferiores.
  - Al navegar hacia arriba, reaparecen progresivamente los elementos previos y salen los inferiores.
...
- La cabecera no participa del scroll vertical del contenido: la región desplazable incluye solo el área de columnas.
...
### 5.10 Tamaños visibles, leyenda de teclas y contador de selección
...
- Se muestra una **línea fija de ayuda** debajo de las columnas con el literal: `up/down mover . -> entrar . <- volver . Esc salir`.
- El contador de posición del elemento seleccionado se muestra en esa misma línea como sufijo ` [n/N]` (por ejemplo `[7/7]`), nunca en una línea separada.
...
| CA09 | La ruta absoluta se muestra en cabecera superior y permanece visible durante scroll vertical | Verificar al navegar y desplazarse en listados largos |
...
| CA14 | Scroll automático con indicadores `▲`/`▼` y bloques dinámicos visibles en directorios grandes | Navegar hacia abajo y arriba en directorio con muchos elementos |
...
| CA25 | Se muestra línea de ayuda en pantalla principal con contador inline `[n/N]` | Verificar línea de ayuda bajo las columnas sin salto adicional del contador |
...
| D07 — Scroll | Automático + indicadores ▲/▼ + bloques dinámicos visibles | Visibilidad completa del listado sin perder contexto de posición |
...
| D11 — Ruta | Cabecera superior fija, ruta absoluta y fuera del área scrolleable | Máxima claridad de ubicación incluso durante scroll vertical |
...
### ✅ Implementación completada (23–27 de marzo de 2026)
...
| **Renderizador Rich** (3 columnas, header, separadores, scroll, style) | ✅ Completado | 20 tests pasados |
...
| **Línea de ayuda + contador inline en UI** | ✅ Completado | Validado visualmente |
| **Especificación de scroll y cabecera fija en OpenSpec** | ✅ Completado | Archivado en change |
...
- Se refuerza el requisito de cabecera fija: la ruta completa debe permanecer visible durante scroll vertical descendente y ascendente.
- Se formaliza comportamiento de scroll en bloques dinámicos visibles (entrada/salida progresiva de elementos según dirección).
- Se confirma no regresión funcional de navegación por flechas: no cambian transiciones ni semántica de selección.
- Se agrega requisito de UX para mostrar el contador `[n/N]` en la misma línea de ayuda, junto al texto `up/down mover . -> entrar . <- volver . Esc salir`.
...
- `explorer/tests/` — suite de pruebas (77 tests)
- `explorer/pyproject.toml` — configuración del proyecto
- `explorer/README.md` — documentación de usuario

### Archivado en OpenSpec (27 de marzo de 2026)

La especificación ejecutable más reciente ha sido archivada en el workflow OpenSpec como **change completado**:

```
openspec/changes/archive/2026-03-27-inline-selection-counter-and-navigation-help/
```

**Cambios incorporados en spec principal (último change):**
- ✅ Contador de selección inline `[n/N]` en la línea de ayuda
- ✅ Estandarización del texto visible de ayuda: `up/down mover . -> entrar . <- volver . Esc salir`
- ✅ Footer sin línea independiente para el contador
- ✅ Cobertura de pruebas para estados normal y con error

**Artifacts:**
- `proposal.md` — Propuesta de cambio ✅ 
- `design.md` — Diseño detallado ✅
- `specs/inline-selection-status-and-navigation-hints/spec.md` — Delta spec sincronizada ✅
- `tasks.md` — 10/10 tareas completadas ✅

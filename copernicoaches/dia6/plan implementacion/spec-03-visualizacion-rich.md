...
5. **Línea de ayuda y contador** — ayuda rápida de navegación con indicador de posición en una línea fija.
...
- Debe permanecer visible durante cualquier desplazamiento vertical de las columnas (descendente o ascendente).

### 2.4 Invariante de layout (actualización OpenSpec)

- El render debe mantener dos zonas lógicas:
  - **Zona fija:** cabecera (ruta activa) y footer.
  - **Zona desplazable:** únicamente el cuerpo de columnas.
- El scroll nunca puede desplazar fuera de pantalla la ruta activa.
...
- Mostrar una línea fija al final del render principal:
  - `up/down mover . -> entrar . <- volver . Esc salir [n/N]`
  - El contador `[n/N]` aparece solo cuando hay selección válida y siempre en la misma línea de ayuda.
...
- Al navegar hacia abajo, los elementos que salen por arriba dejan de renderizarse y aparecen nuevos elementos por abajo.
...
| `test_header_stays_visible_while_scrolling_down` | La cabecera se mantiene visible al desplazarse hacia abajo |
| `test_header_stays_visible_while_scrolling_up` | La cabecera se mantiene visible al desplazarse hacia arriba |
| `test_selection_counter_is_inline_in_header` | La línea de ayuda contiene el contador `[n/N]` en la misma línea |
| `test_footer_does_not_render_standalone_counter_line` | No existe línea independiente con solo el contador |
| `test_navigation_help_text_visible_with_and_without_error` | La ayuda exacta se mantiene visible con y sin error |
...
| V6b | La cabecera permanece visible durante scroll descendente y ascendente |
...
| V12 | Se muestra línea de ayuda con contador inline `[n/N]` |
...
| CA14 (bloques dinámicos) | Renderizado por bloques visibles al navegar arriba/abajo |
...
- ✅ Cabecera con ruta absoluta fija (no se desplaza con scroll)
...
- ✅ Línea de ayuda en la parte inferior del render con contador inline `[n/N]`
- ✅ **20 tests renderer pasados**
- ✅ Visualización pura (no modifica estado)
- ✅ Bloques dinámicos visibles durante scroll descendente/ascendente (OpenSpec)
...
- Se formaliza como requisito de visualización que la cabecera de ruta no se desplace en scroll vertical.
- Se documenta el comportamiento de bloques dinámicos visibles para navegación abajo/arriba en listados largos.
- Se amplía la validación para cubrir explícitamente no regresión de navegación con flechas al introducir cabecera fija.

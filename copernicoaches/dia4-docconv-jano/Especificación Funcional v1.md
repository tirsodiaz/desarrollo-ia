...
Este documento describe la solución de conversión de documentos bidireccional para el proyecto denominado **Jano**. Se origina en el ejercicio de entrega asistida por IA del taller `dia4-docconv-jano` y en la guía de arquitectura asociada.
...
El objetivo es definir la especificación funcional v1 de la aplicación de conversión de documentos. El documento debe:
...
- registrar supuestos, dudas y riesgos
...
```bash
jano input.docx output.md
jano input.md output.docx
```
...
1. El usuario ejecuta `jano input.docx output.md`.
...
- **Hipótesis**: El nombre oficial del producto es `Jano`.
...
1. ¿El modo MCP debe exponer la herramienta exacta `convert_document` o puede tener otro nombre?
2. ¿Qué nivel de compatibilidad de tablas se espera exactamente en Markdown (GFM, pandoc, otro)?
3. ¿Las imágenes deben ser convertidas a referencias Markdown con placeholder o puede omitirse el contenido?
4. ¿Cuál es la política de soporte de HTML embebido en Markdown?
5. ¿Se requiere mantener el estilo visual exacto en DOCX al convertir desde Markdown?
6. ¿Hay un límite de tamaño de documento para la primera versión?
...
- **Riesgo 5**: El proyecto `docconv` dentro del repositorio está vacío/incompleto, por lo que la versión real del producto disponible es `jano`.

---

## 12. Observaciones finales

Esta especificación funcional v1 se basa en el ejercicio asistido por IA y en la guía de arquitectura existente. El proyecto debe construirse como una aplicación única cuyo núcleo de conversión sea reutilizado por ambos modos de operación.

La implementación de referencia existente en el repositorio es `jano`, que cumple con la mayoría de los requerimientos descritos y puede tomarse como base de validación.

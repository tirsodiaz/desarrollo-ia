...
class TestListDirectory:
    ...
    def test_directory_size_is_aggregated_recursively(self, tmp_path):
        root = tmp_path / "root"
        root.mkdir()
        nested = root / "nested"
        nested.mkdir()
        ...
        root_entry = next(e for e in entries if e.name == "root")
        assert root_entry.is_dir is True
        # El tamaño de carpeta ya no se calcula en list_directory (demasiado lento)
        assert root_entry.size_bytes is None

    def test_file_size_remains_direct_stat_size(self, tmp_path):
        file_path = tmp_path / "single.txt"
        file_path.write_bytes(b"123456")

        entries = list_directory(tmp_path)
        file_entry = next(e for e in entries if e.name == "single.txt")
        assert file_entry.is_dir is False
        assert file_entry.size_bytes == 6

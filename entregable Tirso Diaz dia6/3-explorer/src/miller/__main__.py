...
def run_app(
    ...
    try:
        active_console = getattr(active_renderer, "console", None)
        if active_console is not None and hasattr(active_console, "screen"):
            with active_console.screen(hide_cursor=True):
